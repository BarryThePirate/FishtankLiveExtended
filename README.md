...
- **Sort Inventory by Rarity**: reorders the inventory panel and popup so the rarest items come first — epic, rare, uncommon, then common, with empty slots last — and keeps the order up to date as items come and go. Off by default, togglable in settings
...
## Re-run Mode
Replay a past season's archive footage "as if live". Pick a season, day, and start time (down to the second, in 12- or 24-hour format to match the 12-Hour Clock setting) in the Re-run tab, and the extension plays the archives in real time from that point — the clock keeps ticking between visits, so you can follow a season day by day just like it originally aired. Requires being logged in to fishtank.live with a season pass for archive access (the site lists Season 1 as free).
...
- **Clickable Room Zones**: click a doorway in the video to follow someone into the next room — zones are built in for Seasons 1 and 3, and hovering one shows whether that room is on air or a countdown to when its footage resumes. Togglable in settings
...
- **Clock Controls**: pause the re-run clock at any time, nudge it backwards or forwards (±1m / 5m / 1h) with the buttons in the header bar and player, or with the arrow keys (**←/→** = 5s, and **Space** pauses/resumes — works in fullscreen). Turn off "Clock Runs While Away" so time only passes while you're on the site; Clear Re-run wipes the start point when you're done. Player controls auto-hide while you watch and reappear on mouse movement
- **Sidebar Panel**: a Re-run panel in the site's left sidebar, styled like the site's own, with the season, live clock, pause, Open Player, Share, and Clear controls — collapsible like its neighbours. Togglable in settings
- **Share Codes**: the Share button in the player copies a link to the exact moment and room you're watching (e.g. `fishtank.live/#FTL1-s03-D11-181745-kitchen` — readable, and the same real moment for everyone). Opening someone's link plays that moment as a temporary "shared moment" — it never changes your own re-run, and Exit Shared Moment takes you back to it. Codes can also be pasted in the Re-run settings tab
